...
const projectsTr = [
  {
    ...
    description:
      'TeachWork, eğitim ve iş ortamları için tasarlanmış bir görev ve ekip yönetimi web uygulamasıdır. Kullanıcılar kayıt olup giriş yapabilir, odalar oluşturup oda kodu ile katılabilir, bu odalarda görev atayıp durumlarını güncelleyebilir. Odalarda sohbet ve oylama yapılabilir; gruplar oluşturulup yönetilebilir. Uygulama, Google Gemini ile entegre bir yapay zeka asistanı sunar: hem genel sohbet hem de görevlere eklenen PDF dosyaları üzerinde soru-cevap ve analiz yapılabilir. Raporlama modülü ile görev ve çalışma sonuçları raporlanabilir; tüm bu işlevler REST API ve SpringDoc OpenAPI ile dokümante edilmiştir.',
  ...
  },
  {
    ...
    description:
      'Çalışanların yıllık izin taleplerini oluşturduğu, birim yöneticilerinin talepleri onayladığı/reddettiği ve kalan izin günlerini yönettiği tam kapsamlı bir web uygulaması geliştirdim. Sistemde rol tabanlı erişim (Birim yöneticisi, Çalışan), birim ve davet kodu ile üyelik, izin talebi oluşturma ve onay akışı, personel listesinde kalan izin günü güncelleme ve arama/filtreleme özellikleri bulunmaktadır. Sisteme sadece kullanıcı hesabı ile kayıt olunur, talep dahilinde sistem admini rol ataması gerçekleştirir. Backend tarafında ASP.NET Core Web API, Entity Framework Core ve PostgreSQL kullandım; kimlik doğrulama JWT Bearer ile sağlanmaktadır. Frontend tarafında Nuxt (Vue 3) ile SPA yapısında, Tailwind CSS ile responsive arayüz tasarladım. RESTful API tasarımı ve Swagger ile dokümantasyon uygulandı.',
